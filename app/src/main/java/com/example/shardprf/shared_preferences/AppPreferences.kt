package com.example.shardprf.shared_preferences

import android.content.Context
import android.content.SharedPreferences
import com.google.gson.Gson

class AppPreferences private constructor(context: Context, DATABASE_NAME: String) {
    ...
    // this for to set user login or not
    fun login(context: Context, connected: Boolean) {
        sharedPreferences = context.getSharedPreferences("MyAppName", 0)
        val editor = sharedPreferences.edit()
        editor.putBoolean("connected", connected)
        editor.commit()
    }

    // this for to get user login or not
